...
{
public:
    PresentationStep(Device const &device, CommandBufferSubmitter commandBufferSubmitter, CommandPool drawCommandPool, vk::Queue queue, Transferer &transferer);

    uint32_t getCurrentIndex();
    void destroySwapchainKHR();
    void rebuildSwapchainKHR(vk::SurfaceKHR surfaceKHR);
    void updateImages(std::vector<CompleteFrameBuffer> framebuffer, uint32_t indexBackTexture);
