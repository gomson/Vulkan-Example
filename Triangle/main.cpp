...
void buildPrimaryCommandBuffers(RenderPass &renderPass,
    ...
    // This pass will only use seconday command buffer
...
int main(int argc, char *argv[])
{
    ...
    PipelineTriangle pipeline(device, pipelineLayout, renderPass);
    ...
    vk::CommandBufferAllocateInfo allocatePrimaryBufferInfo(
                primaryBufferPool, vk::CommandBufferLevel::ePrimary, swapchainKHR.getImageCount());

    vk::CommandBufferAllocateInfo allocateDrawBufferInfo(
                drawBufferPool, vk::CommandBufferLevel::eSecondary, swapchainKHR.getImageCount());

    std::vector<vk::CommandBuffer> primaryCommandBuffers = device.allocateCommandBuffers(allocatePrimaryBufferInfo);
    std::vector<vk::CommandBuffer> drawBuffers = device.allocateCommandBuffers(allocateDrawBufferInfo);
